...
Data Pull is a product for building enterprise-level data integration and data transformations solutions. Data Pull can work alone or in concert with other solutions to address complex business needs. Data Pull can extract and transform data from a wide variety of sources such as  data files, flat files, and relational data sources,NoSQL stores and then load the data into one or more destinations on a scheduled basis.

Data Pull includes a rich set of built-in tasks and transformations; tools for constructing data migrations. You can use the graphical interface to create solutions without writing a single line of code; or you can contribute to the our code base to meet your custom data needs, which can be deployed as a JAR file.

## DataPull Architecture
![Screenshot](docs/architecture_diagram.png)
...
- ### master_application_config.yml(present in project root directory) configuration parameters
  ...
  - <b>datapull.application.region(Optional)</b> : This specifies the regions supported by AWS. If you specify this value, you don't have to separately specify region for s3 buckets and emr cluster. This parameter is <b>mandatory</b> if you don't specify emr region and s3 region in api config. </br>

  ##### ```datapull.logger.s3``` node:-
  - <b>datapull.logger.s3.loggingfolder</b> : This parameter is used to store spark logs in s3. Give a valid S3 bucket path to store logs. For example {bucket_name}/datapull/logs/ is a valid s3 path. As of now this is a mandatory. </br>

  ##### ```datapull.logger.cloudwatch(Optional)``` node:-
  - Required only if you want DataPull core to put log events into cloudwatch. DataPull api uses ```api/src/main/resources/logback-spring.xml``` for configuring logging. For DataPull api component to put logs into cloudwatch, please enable cloudwatch logging in ```api/src/main/resources/logback-spring.xml```. ```datapull.logger.cloudwatch.accessKey``` and ```datapull.logger.cloudwatch.secretKey``` needs to be specified only if the machine running the application(core and api part) doesn't have the desired IAM roles.

  ##### ````datapull.logger.mssql(Optional)```` node:-
  - Required only you want to log events into mssql. In case you want to use this feature please specify all the paramters required within this node.</br>

  ##### ````datapull.email.smtp(Optional)```` node:-
  - This is required if you want to specify task completion mail from DataPull. If you want to use SMTP server to send you emails, values of all the properties within this node has to be specified. You can also configure AWS SES to send emails.</br>

  ##### ````datapull.email.ses(Optional)```` node:-
  - This property configures aws ses to send DataPull task completion email. ```email``` property within this node is the AWS SES verified email. ```datapull.email.ses.accessKey``` and ```datapull.email.ses.secretKey``` needs to be specified only if the machine running the application(core and api part) doesn't have the desired IAM roles.</br>
...
- ### DataPull API configuration paramters(api/src/main/resources/application-${env}.yml)
  ...
  ##### ````datapull.api```` node:-
  - <b> 1) datapull.api.s3_bucket_name</b>: Specify DataPull S3 bucket name here. This is the bucket where DataPull input jsons and logs are stored. </br>
...
### DataPull App Structure
```DataPull``` api is divided into two components i.e. ```api``` and ```core```. Core contains the actual spark logic that moves data across databases. It is the component that actually does the data transfer. Api contains code that allows accessing DataPull services. It exposes REST endpoints which helps you in the submitting input json. It internally schedules the job, saves DataPull configuration into S3 and manager EMR lifecycle for spark job.

## Configure DataPull
```master_application_config.yml``` under project root directory contains configuration that is common to both core and api. Change following properties within ```master_application_config.yml```<br/>
- ### master_application_config.yml configuration parameters
  <b>datapull.application.region(Optional)</b> : This specifies the regions supported by AWS. If you specify this value, you don't have to separately specify region for s3 buckets and emr cluster. This parameter is <b>mandatory</b> if you don't specify emr region and s3 region in api config. </br>

  ##### ```datapull.logger.s3``` node:-
  - <b>datapull.logger.s3.loggingfolder</b> : This parameter is used to store spark logs in s3. Give a valid S3 bucket path to store logs. For example {bucket_name}/datapull-opensource/logs/ is a valid s3 path. As of now this is a mandatory. </br>
