...
class Migration extends SparkListener {
  ...
  def migrate(migrationJSONString: String, reportEmailAddress: String, migrationId: String, verifymigration: Boolean, reportCounts: Boolean, no_of_retries: Int, custom_retries: Boolean, migrationLogId: String, isLocal: Boolean, preciseCounts: Boolean, appConfig: AppConfig, pipeline: String): Map[String, String] = {
    ...
    try {
      ...
      if (destinationMap("platform") == "cassandra") {
        ...
        // dataframeFromTo.dataFrameToKafka(destinationMap("bootstrapservers"), destinationMap("schemaregistries"), destinationMap("topic"), destinationMap("keyfield"), destinationMap.getOrElse("keyserializer", null), destinationMap("valueserializer"), destinationMap.getOrElse("keyschema", null), destinationMap("valueschema"), destinationMap.getOrElse("keystorepath", "null"), destinationMap.getOrElse("truststorepath", "null"), destinationMap.getOrElse("keystorepassword", "null"), destinationMap.getOrElse("truststorepassword", "null"), destinationMap.getOrElse("keypassword", "null"), sparkSession, dft)
        dataframeFromTo.dataFrameToKafka(spark = sparkSession, df = dft, valueSchemaAsAvroJson = destinationMap("valueschema"), valueField = "value", topic = destinationMap("topic"), kafkaBroker = destinationMap("bootstrapservers"), schemaRegistryUrl = destinationMap("schemaregistries"), keyField = Some(destinationMap("keyfield")), headerField = Some(destinationMap("headerfield")))
  ...
  def jsonSourceDestinationToDataFrame(sparkSession: org.apache.spark.sql.SparkSession, platformObject: JSONObject, migrationId: String, jobId: String, s3TempFolderDeletionError: StringBuilder, pipeline: String): org.apache.spark.sql.DataFrame = {
    ...
    if (platform == "mssql" || platform == "mysql" || platform == "oracle" || platform == "postgres" || platform == "teradata") {
      ...
      dataframeFromTo.kafkaToDataFrame(propertiesMap("bootstrapservers"), propertiesMap("topic"), propertiesMap("offset"), propertiesMap("schemaregistries"), propertiesMap.getOrElse("keydeserializer", "org.apache.kafka.common.serialization.StringDeserializer"), propertiesMap("valuedeserializer"), propertiesMap.getOrElse("s3location", appConfig.s3bucket.toString + "/datapull-opensource/logs/"), propertiesMap.getOrElse("groupid", null), propertiesMap.getOrElse("requiredonlyvalue", "false"), propertiesMap.getOrElse("payloadcolumnname", "value"), migrationId, jobId, sparkSession, s3TempFolderDeletionError, propertiesMap.getOrElse("keystorepath", "null"), propertiesMap.getOrElse("truststorepath", "null"), propertiesMap.getOrElse("keystorepassword", "null"), propertiesMap.getOrElse("truststorepassword", "null"), propertiesMap.getOrElse("keypassword", "null"))
    }
