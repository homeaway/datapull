IF NOT EXISTS(SELECT * FROM sys.tables WHERE name = 'DataPull_Users_Source')
  Begin
  CREATE TABLE dbo.DataPull_Users_Source (
    UserId int PRIMARY KEY ,
    UserGuid varchar(32),
    PasswordSalt varchar(128),
    Password varchar(75),
    PasswordEncryption varchar(32),
    PasswordResetFlag bit,
    PasswordModifiedDate datetime2
  )
  END;


IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 1)
BEGIN
INSERT INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(1, '8f719ace879743268e5c29edf3fc889b', 'USED', 'UMS', 'UMS', 1, '2015-09-29 15:48:00.072')
END;

IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 2)
BEGIN
INSERT INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(2, '067028f8498249a9adb27fa6df67e257', 'USED', '$2a$04$uTVvazCWID661gZL.zwOgufo/HpKB4qaYoYtylCtWjRFhguW/sTXq', 'B_UMS_NORMAL', 1, '2014-03-25 16:21:51.417')
END;

IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 3)
BEGIN 
INSERT  INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(3, 'fd1a20888b524c938f9d05fd26f9f150', 'UNUSED', '$2a$04$g4b/qc7nTlBFbN.DZCr2DeZOz9HYe/0ohjWbDKJllbEhiRNGaPBqS', 'B_UMS_NORMAL', 1, '2014-03-25 16:21:51.423')
END;

IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 4)
BEGIN
INSERT INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(4, '7f8e18c099174c268595c2a277624fe1', 'UNUSED', '$2a$04$fUUE1.IHO.i2E8FJ5XAnt.3t22RHGElPRx9ALipAAh8yRfHJIKrmi', 'B_UMS_NORMAL', 1, '2014-03-25 16:21:51.437') 
END;

IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 5)
BEGIN
INSERT  INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(5, '9ad63308062e49f4b888284af9fc0ff8', 'UNUSED', '$2a$04$pU0HqN3LAR6izBbzXeVpo.RetuKORGvAo/dxvlXpXDCaNJfxm/lMW', 'B_UMS_NORMAL', 1, '2014-03-25 16:21:51.437')
END;

IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 6)
BEGIN
INSERT INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(6, 'eac137afa4c343a6817d72784c43e3b0', 'UNUSED', '$2a$04$h3K2cohvNBLTv46gQwV/5ePCtdkXHtboX.0MVixW2QuNHrf/B.DYO', 'B_UMS_NORMAL', 1, '2014-03-25 16:21:51.453') 
END;

IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 7)
BEGIN
INSERT  INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(7, '17de72b2ca354b8b9a72c023477e13c7', 'UNUSED', '$2a$04$P7WlNwP79bwPvyrB0kywxeh3T3RRMla6anSQ0Y8EFOfnmf6yjaVw2', 'B_UMS_NORMAL', 1, '2014-03-25 16:21:51.466')
END;

IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 8)
BEGIN
INSERT  INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(8, 'fe3c8f650f25415aaced01ad8bbe2899', 'USED', '$2a$04$/1CXd4QY9ijO6CEa896IbemXM7ZOQ9rlonM1e8RWt8y4qGbex6Woq', 'B_UMS_NORMAL', 1, '2014-03-28 22:23:22.557')
END;

IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 9)
BEGIN
INSERT  INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(9, 'fe02a99b51014a3fae0248d55bc4ab29', 'UNUSED', '$2a$04$yiS/1au5oqDZSkLJtncp6.uxe7TEnw/gmWuVgMAQEbiZ7N2EXfeiu', 'B_UMS_NORMAL', 0, '2014-03-25 16:21:51.475')
END;

IF NOT EXISTS(SELECT * FROM dbo.DataPull_Users_Source WHERE UserId = 10)
BEGIN
INSERT  INTO dbo.DataPull_Users_Source
(UserId, UserGuid, PasswordSalt, Password, PasswordEncryption, PasswordResetFlag, PasswordModifiedDate)
VALUES(10, '1af7a0c6fc31468da51c7085348e5060', 'USED', '$2a$04$3hyudHGxYDDLsTfV8tFZLunl0tsiHRsXzd.7mOBmzfj1i2k2gNRDS', 'B_UMS_NORMAL', 0, '2014-03-25 16:21:51.477')
END;